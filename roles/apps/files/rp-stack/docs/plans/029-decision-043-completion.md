...
**Результат 2026-09-03.** Human narrator selection закрыт. Первичный clean
candidate выиграл у сохранённых legacy anchors `11–0` при одной ничьей. После
непринятой коррекции `5–5` при двух ничьих финальный blind раунд сравнил две
clean-версии без legacy: универсальный narrator contract выиграл `8–1` при трёх
ничьих. Владелец принял revision `89bcc7f4409155c28601fa03a10265af468bbaaa`
как новый канонический source. Общие правила не содержат имён мира, персонажей
или способностей; scenario-файл владеет только фактами конкретного opening.
Exact 17-turn proof использовал только `openrouter` →
`openai/gpt-5.6-luna-pro`, без fallback/repair/NVIDIA, и не изменил production.
Остаточное проседание прозы в последних сложных боевых ходах принято как
неблокирующее наблюдение для 6.2–6.3, а не повод ещё раз менять этот A/B.
Полные границы и раскрытый итог находятся в
[evidence](../decisions/evidence/043-narrator-human-ab-2026-09-03.md).
...
**Продолжение 2026-09-11, по поручению владельца «Делай сам дальше».** Codex
начал основную Party и контрастный preset с паузами между ходами. Это отдельный
agent-played acceptance, не человеческая оценка и не новый A/B. Уже на первых
ходах applied image показал `0/6` сохранённых runtime Lore: exact V4 Pro route
обрывал JSON в разрастающемся заголовке. Контрольные воспроизведения выделили
проблемную корневую `oneOf` и затем ошибки атрибуции/домысливания. Исправление
сохраняет exact route, бюджеты и строгую Gateway-валидацию. Реальная операция
PlayerCorrection выявила такой же обрыв в target; её wire schema и инструкция
ограничены одной точной целью. Результаты, неудачные промежуточные варианты,
accepted decisions и последующие сцены записаны в
[evidence Lore/Correction](../decisions/evidence/043-lore-endurance-2026-09-11.md).
Ни §6.2 целиком, ни длинный §6.3 этим коротким проходом не закрыты.
