...
ребилда. Отдельному глобальному треку остаются path-aware GitHub CI, повтор
Gateway pytest в образе, повтор полного workflow после merge и дублирование
policy-hook. Они не являются основанием добавлять в ребилд зеркальные tests,
guards, registry entries, Wiki-страницы или skills.
