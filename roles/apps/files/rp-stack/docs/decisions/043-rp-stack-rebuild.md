...
**База проверки.** Исходники и история сверены с `origin/main @ ebc0219`,
live-счётчики — read-only на применённом `e069670` 28 августа 2026 года. Код
`rp-gateway/app`, tests и repository validator между `e069670` и `ebc0219` не
менялись (`git diff --stat` по этим путям пуст), поэтому ранние статические и
live-измерения остаются в силе.
...
4. **У новой RP-сюиты есть жёсткий бюджет.** К cutover весь автоматически
   ...
   **30 секунд**, и тот же полный прогон не повторяется во втором образе.
   ...
   checks, ручная/live-приёмка и ровно 12 blind A/B anchors. PR указывает
   before/after вручную; отдельный LOC-validator для контроля бюджета запрещён.
...
### Граница с глобальным треком упрощения

Перестройка общих `AGENTS.md`, Devkit, hook'ов, `scripts/ci.ps1`, GitHub workflow,
ADR registry и repository validators выполняется отдельно, потому что затрагивает
не только RP. Там должны быть решены path-aware CI, повтор Gateway pytest в
образе, повтор полного workflow после merge, дублирование policy-hook и
размножение одного контракта по нескольким prose-поверхностям. До завершения
этого трека существующие общие проверки могут запускаться, но они не являются
основанием добавлять в ребилд новые зеркальные tests, guards, registry entries,
Wiki-страницы или skills.
