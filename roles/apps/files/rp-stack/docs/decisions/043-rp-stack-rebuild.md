...
**База проверки.** Исходники сверены с `origin/main @ e069670`, live-счётчики —
read-only на применённом `e069670` 28 августа 2026 года. Код `rp-gateway/app`
между `e16a402` и `e069670` не менялся (`git diff --stat` по этому пути пуст),
поэтому ранние статические измерения остаются в силе.
...
2. **Снять полный evidence-архив.** Не выборка, а immutable party-closure:
   ...
   Охват:
   - `party_cac70558b50a` — 17-ходовая партия `day-watch-moscow-v2`, baseline
     прозы V2 и полная typed Lore chain Decision 042: call `566` → card `286` →
