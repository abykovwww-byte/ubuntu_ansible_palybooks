...
поскольку S1 меняет `codex-skills/`. Подготовить и проверить общий endpoint
закрытия. Активные legacy-партии `awareness` закрыть по §B.6 сразу после
применения новой версии и до пользовательского HTTP smoke: до деплоя этот
endpoint ещё отсутствует, а ручная правка SQLite запрещена.
...
7. **Wave 3 / deploy.** Закрыть активные legacy-партии `awareness` — те, у которых нет
   ...
   применения новой версии и до пользовательского HTTP smoke ни одна партия
   `awareness` без снапшота контракта не остаётся
...
Затем продолжить: подтвердить, что сервис завершился и контейнер Gateway
пересобран; закрыть через новый endpoint активные legacy-партии из шага 7;
только после этого проверить, что партия обоих типов отвечает по HTTP.
...
Отдельно по шагу 7: число активных партий `awareness` без записи в
