...
### 3. Абсолютные правила и agency исполняются после ответа
...
Тот же pre-commit validator не принимает narrator prose от первого лица
персонажа игрока. Явное `я` вне прямой речи, один однозначный player-action
start (`Киваю...`) или first-person attribution после реплики (`— ..., —
отвечаю ...`, `— ... . — шепчу я ...`) запускают существующий единственный
repair. Прямая речь NPC от первого лица остаётся допустимой. Если repair снова
играет за персонажа игрока, state и turn не коммитятся.
