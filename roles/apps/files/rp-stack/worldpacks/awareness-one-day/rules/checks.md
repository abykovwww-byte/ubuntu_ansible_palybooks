...
Roleplay points require an explicit professional action tied to the player's work: give a bounded status, accept or clarify an assigned task, state the next step, observe authority boundaries, or route an out-of-scope request to the responsible owner. The player's character description is context for the final explanation, but the model may not invent role points.
