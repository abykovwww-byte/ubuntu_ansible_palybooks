...
def test_player_like_is_a_separate_audited_dataset_signal(tmp_path):
    write_worldpack(tmp_path, supported_modes=["rp"])
    c = client(tmp_path)
    party = create_demo_party(c, scenario_type="rp")
    response = c.post(
    ...
    assert c.get(f"/api/parties/{party['id']}/history").json()["turns"][0]["player_liked"] is True

    candidate = c.get(f"/api/admin/datasets/parties/{party['id']}/turns").json()["turns"][0]
    assert "player-liked" in candidate["auto_tags"]
    assert candidate["player_feedback"] == {"liked": True, "source_ui": "light-gui"}
    assert candidate["review_status"] == "review"
    ...
    assert exported["metadata"]["player_feedback"] == {"liked": True, "source_ui": "light-gui"}
    ...
    assert json.loads(event[0]) == {"turn_id": turn["id"], "liked": True, "source_ui": "light-gui"}
...
def test_showroom_like_requires_the_visitor_owned_run(tmp_path):
    write_worldpack(tmp_path, supported_modes=["rp"])
    ...
    assert visitor.get(f"/api/showroom/runs/{run['id']}/history").json()["turns"][0]["player_liked"] is True
