...
Приложение управляется элементом `use-framework` роли `apps`. IaC клонирует ровно commit `e4e621903019af855cb0487191afad740470f375`, создаёт server-only `.env`, собирает образ и запускает Compose. По решению владельца сервис публикуется как `0.0.0.0:8765`, включая LAN и Tailscale; nginx и DNS не создаются.
...
Обновление выполняется заменой `use_framework_repo_version` на полный принятый SHA приложения. Перед изменением модели создать backup. Revision `e4e621903019af855cb0487191afad740470f375` не меняет runtime относительно `caaa67346e456ff1bd042ff3e7cf80d9bf33f53b`: его delta состоит только из трёх Markdown-файлов и закрепляет risk-based проверки, отказ от контейнерного `pytest` как pre-deploy gate и минимальный post-apply health/API smoke. Включённый revision `caaa67346e456ff1bd042ff3e7cf80d9bf33f53b` сводит EDR, AD и Assets в доказательный канон: latest-per-source устраняет двойные current instances, карточки показывают identity/fields, production mappings сохраняют ограниченный набор EDR/AD-полей, а fail-closed migration доставляет T2-аудит источников и модели. Exact Assets match обогащает только `sys_erp`; AD остаётся inventory-only; неподтверждённые BC/e1/Tumar связи оформлены как gaps, а не дорисованы. Он сохраняет presentation-изменения revision `d501d705bfbb559b3ae52495f3aea0b6f4ede97e` и предыдущие доказательные миграции.
