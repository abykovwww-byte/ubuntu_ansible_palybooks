...
Говорящий не получает имя адресата. Сравнение, предположение и совпадение по
времени не превращаются в подтверждённое действие либо причину события.
...
Wire schema Lore остаётся плоским strict JSON-объектом с обязательными полями и
nullable draft-полями, без корневого `oneOf`: на exact `baidu/fp8` его сочетание
с nullable-полями в проверенных сценах вызывало незавершающийся заголовок.
Связь полей с `result` проверяет Gateway: `no_candidate` требует null во всех
draft-полях, а `draft` — заполненную карточку. Невалидная комбинация отклоняется
до записи.
...
### Clean Atomic PlayerCorrection

PlayerCorrection также получает плоскую strict schema без корневого `oneOf`:
проверенный exact route с прежней схемой зацикливался на поле `target_slot`.
Цель копируется из одного переданного candidate, без соединения нескольких IDs
и без замены точного claim широким RAW hint. Правка заменяет только выбранный
`before`; прежний предел `after` в 600 символов передаётся модели в JSON Schema
и остаётся обязательным при приёме. Gateway по-прежнему проверяет комбинации
`draft/no_target`, `replace/retract`, точный target и актуальность каталога.
Несколько утверждений не превращаются в массовую перезапись RAW: модель
выбирает одну цель, владелец рассматривает before/after и явно принимает либо
отклоняет предложение. Принятая overlay действует ровно на следующий prompt;
исходный RAW не переписывается.
...
Для пачки из восьми ходов Gateway задаёт `max_tokens=640` и принимает не более
2 000 символов прозы. Для archive-пачки — `max_tokens=1536` и не более 6 000
